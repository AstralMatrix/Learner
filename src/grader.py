...
class Grader(object):
    '''A simple object to be used by the main form to grade and
        display questions (data objects) to help remove the QuizQueue
        overhead. Utilizes the QuizQueue for storing and getting data objects'''

    def __init__(self, file_paths: List[str]):
        # Load in all data objects from the given files.
        self.__raw_data: List[DataObject] = FileReader.read_files(file_paths)

        if self.__raw_data is None:  # Display error data object if loading failed.
    ...
    def get_display_question(self, segment_idx: int) -> str:
        '''Get the segment of a data object. The given segment represents
    ...
    def get_display_answer(self) -> str:
        '''Display the entire data object. This represents the answer to
            the question to be displayed.
